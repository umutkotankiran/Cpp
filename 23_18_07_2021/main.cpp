...
2 - Kodun okunması ve yazılması ve değiştirilmesi zorlaşıyor.Bu çok önemli 
	...
	3 - Hatayı tespit eden kod ile, hatayı işleyecek(müdahale edecek) kod arasında doğrudan bir bağlantı yok.
		...
		Bu fonksiyon bu işi yaparken func çağırmış, o başkasını çağrmış, o öbürünü çağırmış...
		f1 -> f2 -> f3 -> f4 -> f5 ...
...
BIR EXCEPTION YAKALANAMAZSA PROGRAMIN SONLANDIRILMASI SONUCUNU DOĞURUYOR.

SENTAKS
throw expr;

throwu bir ifade izliyor.Bu ifade çoğunlukla bir sınıf türünden ifade ama başka nesnelerde olur.
Programın akışı bu throw deyimine gelince, akış bu throw deyimini içeren functan çıkacak ve bu hatanın yakalanıp işlendiği yere, 
...
using terminate_handler = void(*)(); veya
...
set_terminate çağırıp ona kendi oluşturduğumuz func ın adresini gönderirsek artık bu çağrıdan sonra terminate abortu çağırmak yerine 
...
Set terminate ile terminate davranışı değiştirildiyse, abort yerine set terminate e gönderilen function çağrılır.
...
Bir try bloğu oluşturarak ile bu blok içinde çalışan kodlardan gönderilecek hata nesnesini yakalamaya aday oluyor.
...
4 - Eğer catch int e girdiyse daha sonra hiçbir catch e girmeyerek "main devam ediyor" dan devam eder.
...
Yani böyle bir throw ifadesi aslında derleyicinin böyle bir nesne oluşturması anlamına geliyor.Gönderilen hata nesnesi
...
VERIYI KOPYALAMA YOLUYLA GÖNDERIP, REFERANS YOLUYLA YAKALIYORUZ !!!!!!!!!!!!!!!!

NEDEN REREFANS YOLUYLA YAKALAMALIYIZ
------------------------------------
Referans yoluyla yapılmazsa, 
1 - Copy ctor girecek devreye.copy ctor da exception throw edebilir.
2 - Polymorphic hiyerarşi varsa herhangibir sanal function çağırırsak, virtual dispatch devreye girmeyecek.
	Çünkü object slicing olacak.Tür bilgisi gidecek.

Aşağıdaki gibi olmalı yani
...
void func()
{
	throw Myexception{};	// Copy Ellision devreye girdi.
}
...
(getchar ile durdurduk burada olayı anlamak için)
...
Yine çalışır. Çünkü bad_alloc, exceptiondan türemiş bir sınıf
...
exception cought : invalid string position
...
class bad_date : public std::logic_error{	//bad date i biz uydurduk
};

int main()
{
	throw bad_date{}; // NEDEN SENTAKS HATASI? Compiler diyorki delete edilmiş func ı çağırmaya çalışıyorsun
}						// Default Ctor Delete edilmiş.Nedeni aşağıda
...
Terminate edecek yakalanmadı hata

---------------------------------------------------------------------------------------------------------------------

INHERITED CTOR KULLANILABİLİRDİ.

İnherit ctorda, bad_date ile çağrı yapsak bile logic_error a argüman gönderilecekti.KURAL.
...
Mesela bad_alloc türünden hata yakaladık gerekeni yaptık ve bizde bad_databaseaccess gibi bir hata gönderebiliriz.Daha düğük seviyeden
...
Hatta şimdi f3 teki catch ona ait try dan gelen hatayı yakalayamasın ama f2 yakalasın.
bunu yazalım
...
void f3()
{
	...
	{
		throw ex; // bu exception throw değil. Bu tekrar bir throw etme işlemi yapıyor. tabi bunun bir kopyası çıkarılıyor.
...
foo functionu cagriliyor
foo cagrildi foo hatayi rethrow ediyor
Hata yakalandi (const std::out_of_range&)
...
2 - Sınıf türden nesne ise birde o sınıfın değeri var
	...
	public:
		...
		size_t get_line_no()const;	// hatanın okumanın hangi satırından yapınca gerçekleştiğini söylüyor.
	};
...
Programın akışı buradan çıktığında buraya kadar oluşturulmuş çok sayıda otomatik ömürlü nesne olabilir.Devamı önemli.
...
ÇIKTI
-----
0081FAD3 adresinde nesne olustu
0081F9DF adresinde nesne olustu
0081F8EB adresinde nesne olustu
0081F7F7 adresinde nesne olustu
0081F7EB adresinde nesne olustu
...
2 - Eğer exception yakalandıysa, bunun otomatik ömürlü nesneler üzerinde bir etkisi varmı? KESİNLİKLE VAR.
	...
	YUKARIDAKI KODA BAKALIM.MAIN KISMI AŞAĞIDAKI GIBI OLURSA(EXCEPTIONLAR YAKALANIRSA).DILIN KURALLARI DIYORKI TEKTEK SONDAN BAŞA OLUŞTURULMUŞ TÜM
...
ÇIKTI
-----
00FAFA9F adresinde nesne olustu
00FAF9AB adresinde nesne olustu
00FAF8B7 adresinde nesne olustu
00FAF7C3 adresinde nesne olustu
00FAF7B7 adresinde nesne olustu
00FAF7B7 adresindeki nesne icin destructor cagrildi
00FAF7C3 adresindeki nesne icin destructor cagrildi
00FAF8B7 adresindeki nesne icin destructor cagrildi
00FAF9AB adresindeki nesne icin destructor cagrildi
00FAFA9F adresindeki nesne icin destructor cagrildi
hata yakalandi x = 1
...
EĞER YAKALANMAZSA AŞAĞIDAKİ KOD İLE
int main()
{
	try
	{
...
Destructorlar çağrılmadı çünkü hata yakalanmadı.TERMINATE.
...
BURADA FCLOSE(F); A GITMEYECEK KOD. BAR DAN EXCEPTION GÖNDERILIRSE BILE, AÇILAN DOSYA KAPATILMAYACAK, HATA YAKALANSADA KAPATILMAYACAK.
ÇÜNKÜ BU BIR SINIF NESNESI DEĞIL. DESTRUCTOR ÇAĞRILAMAZ.

Aşağıdaki gibi sınıf olsaydı.Ctoru dosya açıyor, Dtoruda dosya kapatıyor.
