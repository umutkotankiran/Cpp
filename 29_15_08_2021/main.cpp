...
CONTAINER: Veri yapılarını implemente eden sınıf şablonları deniyor. Ör: vector dinamik dizi veri yapısını implemente ediyor.
...
ALGORITMA : Fonksiyon şablonları, ama algoritmaları implemente eden fonk şablonları. sort, find, reverse ...
...
std::unordered_map			-> (C++ 11 de geldi) anahtarın değerle eşlendiği veri yapısı.Buraqdakiler pair gibi anahtar değer çifti
...
Range sözkonusu olduğunda konum dahil ama konum2 dahil değil.
[konum1,konum2)

Burada P1 i kaç kere artırıyorsam, p1-p2 range inde o kadar öğe var demek.
Range kavramı önemli çünkü STL deki algoritmalar buna göre yazılıyor.
Tipik bir algoritmada range parametreleri var.Range olması için minimum 
2 parametre olmalı
...
display_elems(a+4,a+7); bu seferde double dizinin elemanları çağrılacak
						eğer template olsaydı bizimki. ki öyle.
...
namespace std{
	...
	template <typename T> // allocator parametrede varaslında
	class Vector{
	public:
		...
		iterator begin(); //konumları tutuyorlar. Son öğenin konumunu tutuyor
...
int main()
{
	...
	vector<int>::iterator it =  ivec.begin(); // auto burada fayda sağlar
...
Listinde operatorleri overload edilmiş. Listinde iterator nested type var, * ++ funcları var
begin ve end funcları var.Bu kodlar vectörün kodlarıyla birebir aynı değil.Biri bağlı liste
diğeri vector yani elemanlar ardışık contigues

------------------------------------------------------------------------------------------

Iter bir iterator türü ise tüm iteratorlerin != ile karşılaşltırılabileceğine, * operandı
olabileceğine ve ++ operandı olabileceğine güvenerek böyle bir şablon yazarsam, bu koşulları
sağlayan iterator türleri için bu function bu range deki öğeleri yazdıran bir function.
...
ALgoritmanın gücü : Birkaç satırlık func şablonu bu.Bu şablondan yazılacak function, bir int dizide int te arayabilir
					bir bağlı listede tarihte arayabilir, oyn programındaki fighter sınıf türünden nesneler tutan dizide 
...
KURAL 2 : END BOŞTA OLSA DOLUDA OLSA DEREFERENCE ETMEMELIYIZ. 
...
Random access categorisi neler yapabilirse vectörde onları yapabiliyor
...
Nedir bu sınıflar? Aşağıdakiler.
...
STL de de farklı farklı iteratör sınıfları var , vector ün listin ostream in iterator sınıfı gibi. bunların hepsininde birer kategorisi var.
...
InIter Find(InIter beg, InIter end, const T &val) // sonuncu aranacak değer
{
	while(beg != end)
	{
		if(*beg == val)
...
STL agoritmalarının en önemli özelliklerinden biri, yazma range i çoğunlukla başlangıç iteratörğ olarak alınıyor.
...
Algoritmalar iteratörlere göre yazılıyor ve türlerinin ayn ı olması beklenmiyor.
...
int main()
{
	...
	//copy(svec.begin(),svec.end(),slist.begin()); // farklı türler arasonda copy
...
int main()
{
	...
	copy(ivec.begin(),ivec.end(),ilist.begin()); // OK

	vector<int>ivec(20);
	list<Myclass> ilist(100);
...
Bazılarının isminde if sözcüğü geceçek. Bu algoritmalar bir template parametresiyle bir predicate alacaklar ve onun true değer verdiği öğeler üzerinde
...
STL deki arama algoritmaları aranan değerleri bulunamazsa, algoritmaya geçlen range in end konumunu döndürüyor.
...
Aranan değer bulunursa bu değere sahip öğenin konumunu dönüyor bulaamazsa ilgii range in end konumunu döndürüyor.
...
return değeri 1 parametreli ise unary predicate, binary ise binary predicate
...
int main()
{
	...
	//burada bizim yazdpımız fonka vectordeki stringler gönderilecek.
...
CPPREFERENCE SECOND VERSION GİBİ.SINIF NESNESİ FUNC ÇAĞRI OPERATÖRÜNÜN OEPRANDI OLUYOR.FUNCTOR
...
Önce gerçek func yazıp adre sgönderik
sonra sınıf yazdık ...
...
Bu nesne func call operatörünün operandı olacağı için algoritma içinde.
...
elemanı değeri köşeli parantezin içine yazdığımız değerden alınsın demek.
